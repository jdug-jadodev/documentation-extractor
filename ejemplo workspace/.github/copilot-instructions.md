...
- Para desarrollos, impacto, responsabilidades y migraciones usa `docsys_search_documentation` → `docsys_prepare_analysis_context` → herramienta analítica específica; amplía con `docsys_expand_document_context`, después `docsys_investigate_flow` y sólo entonces `docsys_read_source_evidence` con un `evidence_id` autorizado.
- `docsys_locate_capability`, `docsys_trace_business_flow`, `docsys_explain_responsibilities`, `docsys_analyze_change` y `docsys_assess_migration` reutilizan el índice privado sin explorar repositorios.
- `docsys_prepare_proposal` consume preferiblemente `context_id`, `capability_id` y `analysis_id`, y deja borradores trazables en Obsidian.
- Ninguna ausencia documental autoriza a recorrer un repositorio; una ampliación general exige detenerse y solicitar permiso explícito.
