export function selectContext(selection, source) {
    ...
    for (const id of edges.flatMap((item) => item.evidence_ids))
        evidenceIds.add(id);
    ...
    if (missing.length > 0)
        throw new Error(`Contexto esencial ausente: ${missing.join(", ")}`);
