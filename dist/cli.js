...
export async function runCli(argv, options = {}) {
    ...
    try {
        ...
        if (argv.length === 0)
            return await interactiveMenu(packageRoot, validator);
        ...
        if (command === "configurar")
            result = await configure(configPath, validator, parsed.values["no-interactivo"] === true);
            ...
            result = await flow(configPath, validator, parsed.values);
            ...
            result = await migrate(packageRoot, configPath, validator, parsed.values);
        else if (command === "revertir-migracion")
            result = await revertMigration(configPath, parsed.values);
        else if (command === "abrir")
            result = await openVault(configPath, validator);
        else if (command === "estado")
            result = await readRun(configPath, validator, requiredString(parsed.values.run, "--run"), "run.json");
        else if (command === "reanudar")
            result = await resumeStatus(configPath, validator, requiredString(parsed.values.run, "--run"));
        else if (command === "revisar")
            result = await reviewRun(configPath, validator, requiredString(parsed.values.run, "--run"));
        else if (command === "aprobar")
            result = await approveRun(configPath, validator, requiredString(parsed.values.run, "--run"), parsed.values["no-interactivo"] === true);
        else if (command === "publicar")
            result = await publishRun(configPath, validator, requiredString(parsed.values.run, "--run"));
        else if (command === "consultar")
            result = await queryRun(configPath, validator, parsed.positionals[0] ?? "endpoints", parsed.values);
        else if (command === "compartir")
            result = await shareEdition(configPath, validator, parsed.values);
        else if (command === "verificar-edicion")
            result = await verifyEdition(requiredString(parsed.values.ruta, "--ruta"));
        else if (command === "restaurar")
            result = await restoreEdition(configPath, validator, requiredString(parsed.values.edicion, "--edicion"), parsed.values["no-interactivo"] === true);
        else
            throw Object.assign(new Error(`Comando desconocido: ${command}`), { exitCode: 2 });
        emit(result, jsonMode);
        return 0;
    }
    catch (error) {
        const exitCode = typeof error.exitCode === "number" ? error.exitCode : 5;
        const message = error instanceof Error ? error.message : String(error);
        if (jsonMode)
            stdout.write(`${JSON.stringify({ status: exitCode === 3 ? "awaiting_confirmation" : "failed", exit_code: exitCode, error: message })}\n`);
        else
...
async function interactiveMenu(packageRoot, validator) {
    ...
    if (action === "exit")
        return 0;
    if (action === "configure") {
        await runSetup(configPath, validator);
        return 0;
    }
    if (action === "open") {
        const result = await openObsidian(config.vault_root);
        if (!result.opened)
            stdout.write(`${result.manual}\n`);
        return result.opened ? 0 : 2;
    }
    if (await configurationState(config) === "configuration_pending") {
        stdout.write("Configuración pendiente. No se inició ningún análisis automático.\n");
        return 2;
    }
    ...
    try {
        ...
        if (action === "update-one" || action === "update-many")
            result = await update(packageRoot, configPath, validator, values);
        else if (action === "relation")
            result = await relation(configPath, validator, values);
        else if (action === "flow")
            ...
            result = await reviewRun(configPath, validator, requiredString(values.run, "run"));
        emit(result, false);
        return 0;
    }
    finally {
...
async function promptForAction(rl, action) {
    if (action === "update-one" || action === "update-many") {
        ...
        if (!/^s(i)?$/iu.test((await rl.question("\u00bfPreparar análisis local sin IA y sin publicar? [s/N]: ")).trim()))
            return null;
    ...
    if (action === "relation" || action === "flow")
        return { run, desde: (await rl.question("Componente origen: ")).trim(), hasta: (await rl.question("Componente destino: ")).trim() };
    if (action === "proposal")
        return { run, tipo: (await rl.question("Tipo (specification, migration o adr): ")).trim(), solicitud: (await rl.question("Cambio que quieres estudiar: ")).trim() };
    if (action === "query")
        return { run, repo: (await rl.question("Repositorio (vacío = todos): ")).trim() || undefined, categoria: (await rl.question("Categoría (endpoints, dependencies, messages, data, coverage o evidence): ")).trim() || "endpoints" };
    return { run };
}
async function configure(configPath, validator, nonInteractive) { if (nonInteractive)
    throw Object.assign(new Error("Configurar requiere interacción humana."), { exitCode: 3 }); const value = await runSetup(configPath, validator); return value === null ? { status: "cancelled" } : { status: "configured" }; }
async function verify(packageRoot, configPath, validator) { const config = await loadConfiguration(configPath, validator); const grammar = new GrammarManager(join(packageRoot, "assets", "grammars")); return { schema_version: 3, configuration: await configurationState(config), node: process.versions.node, platform: process.platform, architecture: process.arch, grammars: await grammar.verifyAll(), ai_invocations: 0, azure_enabled: config.azure.enabled }; }
async function update(packageRoot, configPath, validator, values) {
    if (values.ia === true)
        throw Object.assign(new Error("Las llamadas IA requieren autorización específica y no están permitidas en esta ejecución."), { exitCode: 3 });
    ...
    if (values["incluir-cambios-locales"] === true) {
        if (repositoryIds.length !== 1)
            throw new Error("Los cambios locales se capturan para un repositorio por vez.");
...
async function migrate(packageRoot, configPath, validator, values) {
    ...
    if (values["dry-run"] === true)
        return plan;
    if (values.aplicar !== true || values["no-interactivo"] === true || !stdin.isTTY)
        throw Object.assign(new Error("Aplicar una migración requiere --aplicar y confirmación humana interactiva."), { exitCode: 3 });
    const rl = createInterface({ input: stdin, output: stdout });
    try {
...
async function revertMigration(configPath, values) {
    if (values["no-interactivo"] === true || !stdin.isTTY)
        throw Object.assign(new Error("Revertir una migración requiere confirmación humana interactiva."), { exitCode: 3 });
...
async function openVault(configPath, validator) { const config = await loadConfiguration(configPath, validator); return await openObsidian(config.vault_root); }
async function reviewRun(configPath, validator, runId) {
    const config = await loadConfiguration(configPath, validator);
    ...
    const serviceModels = new Map(artifacts.snapshots.map((snapshot) => {
        ...
        return [repositoryId, createDocumentModel({ runId, title: `Documentación ${repositoryId}`, snapshots: [snapshot], facts, graph, archifyAvailable: false })];
    }));
...
async function approveRun(configPath, validator, runId, nonInteractive) { if (nonInteractive || !stdin.isTTY)
    ...
    const receipt = await createApprovalReceipt({ runId, candidateRoot: join(root, "candidate-vault"), actor, scope: [runId], humanAction: true });
    await atomicWrite(join(root, "approval.json"), `${JSON.stringify(receipt, null, 2)}\n`);
    return receipt;
}
finally {
    rl.close();
...
async function verifyEdition(path) { const root = resolve(path), manifest = await readJson(join(root, "edicion.json")); await verifyPublicationManifest(root, manifest); return { status: "complete", edition_id: manifest.edition_id, files: manifest.files.length }; }
async function restoreEdition(configPath, validator, edition, nonInteractive) { if (nonInteractive)
    throw Object.assign(new Error("Restaurar requiere confirmación."), { exitCode: 3 }); const config = await loadConfiguration(configPath, validator); const rl = createInterface({ input: stdin, output: stdout }); try {
    const confirmed = /^s(i)?$/iu.test((await rl.question(`¿Restaurar ${edition} como visible? [s/N]: `)).trim());
    await restoreEditionIndex(config.vault_root, edition, confirmed);
    return { status: "restored", edition_id: edition };
}
finally {
    rl.close();
} }
...
function parseRefs(raw, repositoryIds, single) { const refs = {}; if (typeof raw === "string")
    for (const pair of parseList(raw)) {
        const separator = pair.indexOf("=");
        if (separator < 1 || separator === pair.length - 1)
            throw new Error(`Referencia inválida: ${pair}. Use repo=ref.`);
        const id = pair.slice(0, separator).trim(), ref = pair.slice(separator + 1).trim();
        if (!repositoryIds.includes(id))
            throw new Error(`La referencia corresponde a un repositorio no seleccionado: ${id}`);
        refs[id] = ref;
    } if (typeof single === "string") {
    if (repositoryIds.length !== 1)
        throw new Error("--rama solo se admite con un repositorio; use --ramas repo=ref.");
    refs[repositoryIds[0]] = single;
} return refs; }
async function readJson(path) { return JSON.parse(await readFile(path, "utf8")); }
function requiredString(value, name) { if (typeof value !== "string" || value.trim() === "")
    throw Object.assign(new Error(`Falta ${name}.`), { exitCode: 2 }); return value.trim(); }
function optionalString(value) { return typeof value === "string" && value.trim() !== "" ? value.trim() : undefined; }
function parseQueryCategory(value) { const allowed = ["endpoints", "dependencies", "messages", "data", "coverage", "evidence"]; if (!allowed.includes(value))
    throw new Error(`Categoría de consulta inválida: ${value}`); return value; }
function numberOption(value, fallback) { if (typeof value !== "string")
    return fallback; const parsed = Number(value); if (!Number.isSafeInteger(parsed) || parsed < 0)
    throw new Error(`Número inválido: ${value}`); return parsed; }
function emit(value, json) { if (json)
