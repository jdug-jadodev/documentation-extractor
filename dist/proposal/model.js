export function createProposal(input) {
    ...
    if (input.type === "migration") {
        ...
        base.acceptance_criteria.push("Los contratos observados permanecen compatibles durante la transición o cada ruptura queda documentada y aprobada humanamente.", "Las relaciones candidatas o no resueltas que afecten el corte final quedan confirmadas, descartadas o aceptadas explícitamente como riesgo.", "El retiro de la implementación anterior solo ocurre después de verificar la transición y el rollback con evidencia revisable.");
        if (edgeTypes.has("calls_http"))
            base.tests.push("Pruebas contractuales para cada relación HTTP observada que resulte afectada, sin asumir cuerpos ni códigos no extraídos.");
        if (edgeTypes.has("reads_data") || edgeTypes.has("writes_data"))
            base.tests.push("Pruebas de regresión sobre las lecturas y escrituras de datos observadas que cambien de responsabilidad.");
        if (edgeTypes.has("publishes_to") || edgeTypes.has("consumes_from"))
...
function unique(values) { return [...new Set(values.filter((value) => value.trim() !== ""))]; }
