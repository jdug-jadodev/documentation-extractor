...
export async function buildCandidateVault(root, model, graph, serviceModels = new Map(), facts = []) {
...
function renderServiceVisualIndex(repositoryId, architecture, flows, graph, facts) {
...
function renderServiceArchitecture(repositoryId, facts, graph, allFacts) {
    ...
    if (groups.size === 0 && lines.length === 2)
...
function renderEndToEnd(graph, facts, flows) {
...
function renderMicroserviceDocument(graph, facts) {
...
function renderMicroserviceMermaid(graph, facts) {
    const edges = graph.edges.filter((edge) => edge.type === "calls_http");
    if (edges.length === 0)
        return "flowchart LR\n  empty[\"Sin llamadas HTTP detectadas\"]";
    const ids = new Map(), label = (nodeId) => graph.nodes.find((node) => node.id === nodeId)?.label ?? nodeId, id = (nodeId) => { let value = ids.get(nodeId); if (!value) {
        value = `s${ids.size}`;
        ids.set(nodeId, value);
    } return value; };
    const lines = ["flowchart LR"], seen = new Set();
    for (const edge of edges) {
        ...
        lines.push(`  ${id(edge.from)}["${escapeMermaid(label(edge.from))}"] ${arrow}|"${escapeMermaid(relationLabel(edge, facts))}"| ${id(edge.to)}["${escapeMermaid(label(edge.to))}"]`);
    }
    return lines.join("\n");
}
function renderServiceRelations(repositoryId, graph, facts) {
