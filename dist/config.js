...
export async function loadConfiguration(path, validator) {
    const configPath = resolve(path);
    const raw = await readFile(configPath, "utf8");
    if (Buffer.byteLength(raw, "utf8") > 1024 * 1024)
        throw new Error("knowledge.yaml excede el límite de 1 MiB.");
    const document = parseDocument(raw, { prettyErrors: true, strict: true });
    if (document.errors.length > 0)
        throw new Error(`knowledge.yaml inválido: ${document.errors.map((error) => error.message).join("; ")}`);
    const value = document.toJS({ maxAliasCount: 20 });
    validator.assert("config", value);
    const configRoot = dirname(configPath);
    const repositoryIds = new Set();
    const repositories = value.repositories.map((repository) => {
        if (repositoryIds.has(repository.id))
            throw new Error(`ID de repositorio duplicado: ${repository.id}`);
...
export function stringifyConfiguration(config) {
    const lines = [
        "schema_version: 3",
    ...
    if (config.azure.branch !== undefined)
        lines.push(`  branch: ${yamlString(config.azure.branch)}`);
    if (config.overrides !== undefined)
        lines.push(`overrides: ${JSON.stringify(config.overrides)}`);
    return `${lines.join("\n")}\n`;
}
export async function configurationState(config) {
    if (config.setup_status !== "configured" || config.workspace_path === null)
        return "configuration_pending";
    try {
        if (!(await stat(config.workspace_path)).isFile())
            return "configuration_pending";
    }
    catch {
        return "configuration_pending";
...
export async function canonicalizeConfiguredRoot(path) {
    if (!isAbsolute(path))
        throw new Error("La raíz efectiva debe ser absoluta.");
