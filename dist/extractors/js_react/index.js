...
export function createReactPlugin() {
    return new PatternExtractorPlugin({
        ...
        rules: [
            ...
            { id: "react.fetch", version: "1", capability: "http_clients", languages: ["javascript", "typescript", "tsx"], file: /\.[jt]sx?$/iu, pattern: /\bfetch\(\s*([`"'][^`"']+[`"'])(?:\s*,\s*\{[\s\S]{0,300}?method\s*:\s*(["'][A-Z]+["']))?/giu, factKind: "http_client_call", map: (match) => ({ library: "fetch", method: method(pathValue(match[2]), "GET"), path_expression: pathValue(match[1]), target: null }) },
            ...
            { id: "react.package", version: "1", capability: "manifests", languages: ["json"], file: /package\.json$/iu, pattern: /["'](react(?:-dom|-router-dom)?)["']\s*:\s*["']([^"']+)["']/giu, factKind: "package_dependency", map: (match) => ({ package: match[1] ?? null, declared_version: match[2] ?? null }) }
        ]
    });
}
