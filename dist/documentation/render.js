...
export function renderDocument(model) {
    ...
    model.sections.forEach((section, index) => { lines.push(`## ${index + 1}. ${escapeHeading(section.title)}`, ""); for (const paragraph of section.paragraphs)
        lines.push(escapeMarkdown(paragraph), ""); for (const table of section.tables)
        lines.push(renderTable(table.headers, table.rows), ""); if (section.limitations.length > 0) {
...
export function renderSectionView(model, sectionId) {
    const section = model.sections.find((item) => item.id === sectionId);
    if (!section)
        throw new Error(`Sección inexistente: ${sectionId}`);
    const temporary = { ...model, title: `${model.title} — ${section.title}`, sections: [section] };
    return renderDocument(temporary);
}
export function renderRelationshipMermaid(model) {
...
function renderTable(headers, rows) { const safeHeaders = headers.map(tableCell); const actualRows = rows.length > 0 ? rows : [headers.map(() => "No detectado")]; return [`| ${safeHeaders.join(" | ")} |`, `| ${safeHeaders.map(() => "---").join(" | ")} |`, ...actualRows.map((row) => `| ${headers.map((_, index) => tableCell(row[index] ?? "")).join(" | ")} |`)].join("\n"); }
function tableCell(value) { return escapeMarkdown(value).replaceAll("|", "\\|").replace(/[\r\n]+/gu, " "); }
function escapeMarkdown(value) { return value.replace(/<script\b[^>]*>[\s\S]*?<\/script>/giu, "[contenido omitido]").replace(/javascript:/giu, "blocked:"); }
function escapeHeading(value) { return escapeMarkdown(value).replace(/[\r\n#]/gu, " "); }
