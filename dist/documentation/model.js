...
export function createDocumentModel(input) {
    ...
    if (input.facts.length === 0)
        sections.find((section) => section.id === "riesgos")?.limitations.push("Desconocido: el alcance no contiene hechos extraídos.");
...
function factualSection(id, facts, graph, snapshots) {
    ...
    if (id === "decisiones")
        base.paragraphs.push("La documentación factual se publica automáticamente después de validación mecánica. Las ADR, migraciones y decisiones de diseño conservan su carácter de propuesta.");
    if (id === "evidencias")
        base.tables.push({ headers: ["Tipo de hecho", "Cantidad", "Ejemplos de evidencia"], rows: [...facts.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([kind, values]) => [kind, String(values.length), values.slice(0, 5).flatMap((fact) => fact.evidence_ids).join(", ")]) });
    if (id === "anexos")
...
function groupedArchitecture(modules, symbols, field) {
...
function groupSymbolsByRole(symbols) {
    const grouped = new Map();
    for (const fact of symbols) {
        const value = record(fact.value), role = stringify(value.role);
        grouped.set(role, [...(grouped.get(role) ?? []), `${stringify(value.name)} [${stringify(value.symbol_type)}] — ${stringify(value.source_path)}`]);
    }
    return [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([role, values]) => [role, String(values.length), values.join("; ")]);
}
function symbolDescription(value) {
...
function resolvedEndpoints(facts) {
    ...
    return endpoints.flatMap((fact) => {
        ...
        if (fact.kind === "http_endpoint" || value.route_scope === "application")
            return [{ component: fact.component_id, method, path, handler, source_path, status: "ruta completa", evidence: fact.evidence_ids.join(", ") }];
        const matchingMounts = mounts.filter((mount) => {
            if (mount.component_id !== fact.component_id)
                return false;
        ...
        if (matchingMounts.length === 0)
            return [{ component: fact.component_id, method, path, handler, source_path, status: "fragmento; montaje no resuelto", evidence: fact.evidence_ids.join(", ") }];
...
function normalizedSymbol(value) { return value.toLocaleLowerCase("en-US").replace(/(?:router|routes?|[-_.])/gu, ""); }
function joinRoute(prefix, path) { const value = `${prefix}/${path}`.replace(/\/{2,}/gu, "/").replace(/\/$/u, ""); return value || "/"; }
