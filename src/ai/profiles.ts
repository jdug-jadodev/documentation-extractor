...
const AI_ROLES = ["orchestrator", "extraction", "integration", "documentation", "review", "proposal"] as const;
...
export async function generateEffectiveProfiles(options: { packageRoot: string; model: string; outputRoot?: string }): Promise<Array<{ role: string; path: string; source_hash: string }>> {
  if (!/^[A-Za-z0-9._:/-]+$/u.test(options.model)) throw new Error("Identificador de modelo inválido.");
