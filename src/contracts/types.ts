...
export interface SnapshotReader {
  ...
  read(relativePath: string, options?: { maxBytes?: number; signal?: AbortSignal }): Promise<Uint8Array>;
}
...
export interface Inventory {
  ...
  repository_id: string;
  snapshot_id: string;
  projects: Array<{ id: string; root: string; manifest: string; technologies: string[] }>;
  files: InventoryFile[];
...
export interface Scenario {
  schema_version: 3;
  id: string;
  snapshots: Array<{ repository_id: string; snapshot_id: string }>;
  environment: string | null;
...
export interface TaskPacket {
  ...
  conventions: Array<{ statement: string; source: string; author: string | null }>;
  template_sections: string[];
  coverage: Record<string, Coverage>;
  unknowns: string[];
  required_output_schema: string;
  limits: { allow_source_access: false; allow_publication: false; max_input_bytes: number; max_output_bytes: number };
}
...
export interface AgentResult<T = Record<string, unknown>> {
  ...
  payload: T;
  limitations: string[];
  evidence_requests: Array<{ category: string; question: string }>;
}
...
export interface PublicationTarget {
  publish(candidateRoot: string, receipt: ApprovalReceipt, options: { signal?: AbortSignal }): Promise<PublicationManifest>;
}
