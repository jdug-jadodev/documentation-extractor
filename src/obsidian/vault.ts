...
export async function buildCandidateVault(root: string, model: DocumentModel, graph: KnowledgeGraph, serviceModels: ReadonlyMap<string, DocumentModel> = new Map()): Promise<void> {
  ...
  for (const snapshot of model.snapshots) {
    ...
    for (const [file, section] of VIEWS) await atomicWrite(join(serviceRoot, file), renderSectionView(serviceModel, section));
  }
