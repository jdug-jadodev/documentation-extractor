...
export class JavaSpringPlugin implements ExtractorPlugin {
  readonly id = "java-spring";
  readonly version = "2.0.0";
  readonly supported_languages = ["java", "xml"] as const;
  readonly capabilities = { http_endpoints: "implemented", ...supportingPlugin.capabilities } as const;
  readonly rule_versions = { "spring.annotated-endpoint": "2", ...supportingPlugin.rule_versions } as const;
  ...
  async extract(reader: SnapshotReader, component: CandidateStack, options: ExtractionOptions): Promise<ExtractionResult> {
    ...
    for (const entry of entries) {
      ...
      for (const endpoint of annotatedEndpoints(source)) { processed += 1; addFact(reader, component.component_id, entry, source, endpoint.value, endpoint.start, endpoint.end, facts, evidence); }
    }
    const coverage_by_capability: CapabilityCoverage[] = [{ capability: "http_endpoints", status: "implemented", processed, failed, limitations: ["Las expresiones de ruta no resolubles se conservan entre llaves."] }, ...supporting.coverage_by_capability];
...
export function createJavaSpringPlugin(): ExtractorPlugin { return new JavaSpringPlugin(); }

interface LocatedEndpoint { start: number; end: number; value: Record<string, unknown>; }

function annotatedEndpoints(source: string): LocatedEndpoint[] {
  const constants = stringConstants(source);
  const classMappings = classRoutePrefixes(source, constants);
  const result: LocatedEndpoint[] = [];
  ...
  for (const match of source.matchAll(annotation)) {
    ...
    if (/\b(?:class|interface|record|enum)\s+[A-Za-z_$]/u.test(between)) continue;
    ...
    if (parameterClose === null) continue;
    const bodyOpen = source.indexOf("{", parameterClose + 1);
    if (bodyOpen < 0 || /[;=]/u.test(source.slice(parameterClose + 1, bodyOpen))) continue;
    ...
    for (const path of paths) result.push({ start, end, value: { framework: "spring-annotated", method: httpMethod, path: joinPaths(classMappings.get(classInfo.name) ?? "", path), route_scope: "method", handler_expression: `${classInfo.name}.${method[2] ?? "unknown"}`, handler_class: classInfo.name, handler_method: method[2] ?? "unknown", return_type: returnType, reactive: /\b(?:Mono|Flux)\s*</u.test(returnType), source_path: null } });
  }
  return result;
}

function matchingDelimiter(source: string, open: number, left: string, right: string): number | null {
  if (source[open] !== left) return null; let depth = 0, quote: string | null = null, escaped = false;
  for (let index = open; index < source.length; index += 1) { const char = source[index]!; if (quote !== null) { if (escaped) escaped = false; else if (char === "\\") escaped = true; else if (char === quote) quote = null; continue; } if (char === "\"" || char === "'") { quote = char; continue; } if (char === left) depth += 1; else if (char === right && --depth === 0) return index; }
  return null;
}

function classRoutePrefixes(source: string, constants: ReadonlyMap<string, string>): Map<string, string> {
  const result = new Map<string, string>();
  for (const match of source.matchAll(/@RequestMapping\s*(?:\(([^)]*)\))?[\s\r\n]*(?:(?:public|abstract|final)\s+)*class\s+([A-Za-z_$][\w$]*)/gu)) result.set(match[2] ?? "unknown", annotationPaths(match[1] ?? "", constants)[0] ?? "/");
  return result;
}

function annotationPaths(args: string, constants: ReadonlyMap<string, string>): string[] {
...
function stringConstants(source: string): Map<string, string> { const result = new Map<string, string>(); for (const match of source.matchAll(/\b(?:static\s+)?final\s+String\s+([A-Z_$][\w$]*)\s*=\s*([^;]+);/gu)) result.set(match[1] ?? "", resolveString(match[2] ?? "", result)); return result; }
function resolveString(expression: string, constants: ReadonlyMap<string, string>): string { const value = expression.trim(); if (value === "") return ""; return value.split("+").map((part) => { const item = part.trim(); const quoted = /^(?:"([^"]*)"|'([^']*)')$/u.exec(item); return quoted ? quoted[1] ?? quoted[2] ?? "" : constants.get(item) ?? `{${item}}`; }).join(""); }
...
function enclosingClass(source: string, position: number): { name: string; index: number } | null { const match = [...source.slice(0, position).matchAll(/\b(?:class|interface|record)\s+([A-Za-z_$][\w$]*)/gu)].at(-1); return match ? { name: match[1] ?? "unknown", index: match.index ?? 0 } : null; }
function isTestPath(path: string): boolean { return /(^|\/)src\/test\/|(^|\/)(?:test|tests|spec|specs)(\/|$)|(?:Test|Tests|Spec)\.java$/u.test(path); }

function addFact(reader: SnapshotReader, componentId: string, entry: SnapshotEntry, source: string, rawValue: Record<string, unknown>, characterStart: number, characterEnd: number, facts: Fact[], evidence: Evidence[]): void {
