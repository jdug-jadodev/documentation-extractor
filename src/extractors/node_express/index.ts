...
export function createNodeExpressPlugin() {
  return new PatternExtractorPlugin({
    ...
    rules: [
      { id: "express.route", version: "2", capability: "http_endpoints", languages: ["javascript", "typescript"], file: /\.[cm]?[jt]s$/iu, pattern: /\b(app|router)\s*\.\s*(get|post|put|patch|delete|options|head)\s*\(\s*(["'`][^"'`]+["'`])\s*,\s*([\s\S]{1,500}?)\)\s*;/giu, factKind: "http_endpoint_fragment", map: (match) => ({ framework: "express", receiver: match[1]?.toLocaleLowerCase("en-US") ?? "unknown", method: method(match[2]), path: pathValue(match[3]), handler_expression: match[4]?.replace(/\s+/gu, " ").trim() ?? "unknown", route_scope: match[1]?.toLocaleLowerCase("en-US") === "app" ? "application" : "router" }) },
