...
export function redactValue(value: unknown, key = ""): unknown {
  ...
  if (value !== null && typeof value === "object") return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([nestedKey, nested]) => [nestedKey, redactValue(nested, nestedKey)]));
  return value;
}

export function assertNoKnownSecret(text: string): void {
  ...
  for (const pattern of dangerous) { pattern.lastIndex = 0; if (pattern.test(text)) throw new Error("Se detectó un secreto potencial en una salida exportable."); }
}
