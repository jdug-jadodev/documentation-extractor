...
import { loadDocumentationIntelligence, type DocumentationIntelligenceIndex } from "./index.js";

const DEFAULT_BUDGET: ContextBudget = { max_context_tokens: 15_000, max_documents: 40, max_flows: 5, max_symbols: 25, max_documents_per_repository: 15 };

export async function searchDocumentation(runRoot: string, query: string, options: { repository?: string; limit?: number } = {}) {
  ...
  const scored = index.chunks.filter((chunk) => options.repository === undefined || chunk.repository_id === options.repository).map((chunk) => {
    ...
    for (const term of terms) score += metadata.includes(term) ? 12 : body.includes(term) ? 3 : 0;
    if (chunk.endpoint) {
      const endpointText = normalize(`${chunk.endpoint.method} ${chunk.endpoint.path}`);
      if (exact.includes(normalize(chunk.endpoint.path)) || exact.includes(endpointText)) score += 80;
      score += terms.filter((term) => endpointText.includes(term)).length * 35;
    }
    if (chunk.confidence_states.includes("supported")) score += 2;
    return { chunk, score };
...
export async function expandDocumentContext(runRoot: string, seedIds: readonly string[], depth = 2, limit = 60) {
  ...
  while (frontier.length > 0 && visited.size < 5_000) {
    ...
    if (current.level >= Math.min(depth, 8)) continue;
    for (const edge of adjacency.get(current.id) ?? []) {
      if (edge.to === current.id && edge.from !== current.id && !["contains_symbol", "implements", "implemented_by"].includes(edge.type)) continue;
      const candidate = edge.from === current.id ? edge.to : edge.from, nextLevel = current.level + 1;
      if (byId.has(candidate)) { if ((chunkDistance.get(candidate) ?? Number.POSITIVE_INFINITY) > nextLevel) chunkDistance.set(candidate, nextLevel); continue; }
      if (visited.has(candidate)) continue;
      const technicalDegree = (adjacency.get(candidate) ?? []).filter((item) => !byId.has(item.from === candidate ? item.to : item.from)).length;
      if (technicalDegree > 64 && !/(?:Port(?:In|Out)?|Repository|Mapper|Entity|UseCase|Handler)(?:\.|$)/u.test(candidate)) continue;
      visited.add(candidate); frontier.push({ id: candidate, level: nextLevel });
    }
  }
...
export async function prepareAnalysisContext(input: { runId: string; runRoot: string; query: string; intent: AnalysisIntent; repository?: string; budget?: Partial<ContextBudget> }): Promise<CompiledContext> {
  ...
  const selected: DocumentationChunk[] = [], perRepository = new Map<string, number>(), selectedSymbols = new Set<string>();
  ...
  for (const chunk of orderedCandidates) {
    ...
    if (!documents.has(chunk.document_path) && documents.size >= budget.max_documents) continue;
    if ((perRepository.get(chunk.repository_id) ?? 0) >= budget.max_documents_per_repository && !selected.some((item) => item.document_path === chunk.document_path)) continue;
    if (chunk.document_type === "flow" && flows >= budget.max_flows) continue;
    ...
    if (symbols + newSymbols > budget.max_symbols) continue;
    if (tokensUsed + estimated > budget.max_context_tokens) continue;
    selected.push(chunk); tokensUsed += estimated; symbols += newSymbols; for (const symbol of chunkSymbols) selectedSymbols.add(symbol); if (chunk.document_type === "flow") flows++;
...
export async function loadContext(runRoot: string, contextId: string): Promise<CompiledContext> {
  if (!/^context-[A-Za-z0-9]+$/u.test(contextId)) throw new Error("context_id inválido.");
  return JSON.parse(await readFile(join(runRoot, "documentation-intelligence", "context-cache", `${contextId}.json`), "utf8")) as CompiledContext;
}

function priority(chunk: DocumentationChunk, intent: AnalysisIntent): number {
  const weights: Record<AnalysisIntent, RegExp> = { development: /flow|router|contract|method|mapper|repository|data/u, migration: /flow|contract|data|integration|service|architecture/u, impact: /call|caller|contract|integration|data|module/u, responsibilities: /service|architecture|flow|integration|data/u };
  ...
  if (intent === "development") {
    ...
    if (/^(?:class|interface|record|enum)\s+/iu.test(chunk.heading ?? "")) score += 60;
  }
  if (chunk.endpoint !== null) score += 8; if (chunk.repository_id !== "_workspace") score += 2; if (chunk.evidence_ids.length > 0) score += 1;
  return score;
}
function contextualPriority(chunk: DocumentationChunk, intent: AnalysisIntent, expansionDistance: ReadonlyMap<string, number>, seeds: ReadonlySet<string>, queryTerms: readonly string[]): number { const distance = expansionDistance.get(chunk.id), metadata = normalize(`${chunk.heading ?? ""} ${chunk.document_path} ${chunk.source_paths.join(" ")}`), affinity = queryTerms.filter((term) => metadata.includes(term)).length * 15; return priority(chunk, intent) + affinity + (distance === undefined ? 0 : Math.max(20, 300 - distance * 40)) + (seeds.has(chunk.id) ? 1_000 : 0); }
function missingInformation(chunks: readonly DocumentationChunk[], intent: AnalysisIntent): string[] { const types = chunks.map((chunk) => chunk.section_type).join(" "), technical = chunks.flatMap((chunk) => [...chunk.roles, ...chunk.source_paths]).join(" "), missing: string[] = []; if (!/flow/u.test(types)) missing.push("flujo de entrada relacionado"); if (!/data|persist|repository/u.test(`${types} ${technical}`)) missing.push("propiedad o acceso de persistencia"); if ((intent === "migration" || intent === "responsibilities") && new Set(chunks.map((chunk) => chunk.repository_id).filter((id) => id !== "_workspace")).size < 2) missing.push("participación de otros sistemas"); return missing; }
function adjacencyIndex(links: readonly DocumentationLink[]): Map<string, DocumentationLink[]> { const result = new Map<string, DocumentationLink[]>(); for (const edge of links) { if (edge.type === "documents" || edge.status === "unresolved") continue; result.set(edge.from, [...(result.get(edge.from) ?? []), edge]); result.set(edge.to, [...(result.get(edge.to) ?? []), edge]); } return result; }
function dedupeChunks(chunks: readonly DocumentationChunk[]): DocumentationChunk[] { return [...new Map(chunks.map((chunk) => [chunk.id, chunk])).values()]; }
function structuralFirst(chunks: readonly DocumentationChunk[], seeds: ReadonlySet<string>): DocumentationChunk[] {
  const classChunks = chunks.filter((chunk) => /^(?:class|interface|record|enum)\s+/iu.test(chunk.heading ?? ""));
  const pick = (pattern: RegExp, limit = 1) => classChunks.filter((chunk) => pattern.test(chunk.heading ?? "")).slice(0, limit);
  return dedupeChunks([
...
function estimateTokens(value: string): number { return Math.ceil(Buffer.byteLength(value, "utf8") / 4); }
function meaningfulSymbols(values: readonly string[]): string[] { return values.filter((value) => /^[A-Z]/u.test(value) && !/\.(?:java|class)$/iu.test(value) && !/^(?:ServerResponse|MediaType|Mono|Flux|String|Math|Integer|Boolean|List|Logger|PageResponse|Type)\b/u.test(value)); }
function normalize(value: string): string { return value.normalize("NFKD").replace(/[\u0300-\u036f]/gu, "").toLocaleLowerCase("en-US"); }
function tokens(value: string): string[] { return [...new Set(normalize(value).split(/[^a-z0-9_./-]+/gu).filter((token) => token.length > 2))]; }
function queryTokens(value: string): string[] { const result = tokens(value).filter((token) => !["para", "como", "desde", "sobre"].includes(token)); if (result.some((token) => token === "administracion" || token === "administrativo" || token === "administrativos")) result.push("admin"); if (result.some((token) => token === "permiso" || token === "permisos")) result.push("permission", "permissions"); return [...new Set(result)]; }
