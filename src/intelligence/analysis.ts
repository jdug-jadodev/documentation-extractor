...
export async function locateCapability(runRoot: string, query: string) {
...
export async function traceBusinessFlow(runRoot: string, query: string) {
...
export async function explainResponsibilities(runRoot: string, query: string) {
  const located = await locateCapability(runRoot, query), capability = located.matches[0]?.capability;
  if (capability === undefined) return { schema_version: 3, query, status: "unresolved", responsibilities: [], limitations: ["No se localizó una capacidad documental relacionada."] };
...
export async function analyzeChange(input: { runId: string; runRoot: string; request: string; facts: readonly Fact[]; graph: KnowledgeGraph; contextId?: string }) {
...
export async function assessMigration(input: { runId: string; runRoot: string; request: string; facts: readonly Fact[]; graph: KnowledgeGraph; contextId?: string; from?: string; to?: string }) {
...
export function investigationDecision(context: CompiledContext, requestedLevel = 1) {
  const level = Math.max(1, Math.min(requestedLevel, 6));
  if (context.sufficient) return { level: 1, status: "sufficient", next_action: "synthesize", authorization_required: false };
  if (level === 1) return { level: 2, status: "expand_documentation", next_action: "docsys_expand_document_context", authorization_required: false };
  if (level === 2) return { level: 3, status: "directed_analysis", next_action: "reuse indexed technical facts for reachable dependencies", authorization_required: false, limits: { max_dependency_depth: 3, max_files: 8, max_bytes: 262144 } };
  if (level === 3) return { level: 4, status: "source_evidence", next_action: "docsys_read_source_evidence", authorization_required: false, limits: { max_files: 8, max_bytes: 262144 } };
  if (level === 4) return { level: 5, status: "restricted_agent", next_action: "provide only context and authorized evidence", authorization_required: false, tools: [] };
...
function searchable(capability: Capability): string[] { return [capability.id, capability.name, ...capability.aliases, ...capability.entrypoints].map(normalize); }
function tokens(value: string): string[] { return [...new Set(normalize(value).split(/[^a-z0-9_./-]+/gu).filter((token) => token.length > 2))]; }
function queryTokens(value: string): string[] { return [...new Set(tokens(value).map((token) => token === "administracion" || token.startsWith("administrativ") ? "admin" : token).filter((token) => !["para", "como", "desde", "sobre"].includes(token)))]; }
function normalize(value: string): string { return value.normalize("NFKD").replace(/[\u0300-\u036f]/gu, "").toLocaleLowerCase("en-US"); }
function unique<T>(values: readonly T[]): T[] { return [...new Set(values)]; }
function runIdFromRoot(root: string): string { return root.replaceAll("\\", "/").split("/").at(-1) ?? "unknown"; }
async function loadContextLocal(runRoot: string, contextId: string): Promise<CompiledContext> { const { loadContext } = await import("./retrieval.js"); return await loadContext(runRoot, contextId); }
