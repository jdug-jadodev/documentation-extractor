...
export interface DocumentationIntelligenceManifest {
  ...
  run_id: string;
  generated_at: string;
  documents: Array<{ path: string; sha256: string; chunks: string[] }>;
  chunks: number;
  reused_chunks: number;
  metrics: { duration_ms: number; heap_delta_bytes: number; link_count: number };
  repositories: Array<{ id: string; ref: string; commit: string }>;
}
...
export interface Capability {
  ...
  entrypoints: string[];
  flows: string[];
  responsibilities: Array<{ repository_id: string; role: string; basis: "extracted" | "explicit"; evidence_ids: string[] }>;
  legacy_dependencies: Array<{ from: string; to: string; status: ConfidenceState }>;
  contracts: string[];
