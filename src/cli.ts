...
async function shareEdition(configPath: string, validator: ContractValidator, values: Record<string, unknown>) {
  ...
  const sourceRun = await readJson<{ snapshots: Array<{ dirty: boolean }> }>(join(validatedRunRoot(config.config_root, manifest.run_id), "run.json"));
...
async function readRun(configPath: string, validator: ContractValidator, runId: string, file: string) { const config = await loadConfiguration(configPath, validator); return await readJson(join(validatedRunRoot(config.config_root, runId), file)); }
