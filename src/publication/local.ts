...
async function retainOnlyEdition(publicationsRoot: string, editionId: string): Promise<void> {
  for (const entry of await readdir(publicationsRoot, { withFileTypes: true }).catch(() => [])) {
    if (entry.name === editionId || !entry.isDirectory() || entry.isSymbolicLink()) continue;
