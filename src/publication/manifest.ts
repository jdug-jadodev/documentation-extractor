...
export async function verifyPublicationManifest(root: string, manifest: PublicationManifest): Promise<void> {
  for (const file of manifest.files) { const bytes = await readFile(join(root, ...file.path.split("/"))); if (bytes.byteLength !== file.size || sha256(bytes) !== file.sha256) throw new Error(`Archivo de edición alterado o incompleto: ${file.path}`); }
}

async function listFiles(root: string): Promise<string[]> { const result: string[] = []; for (const entry of await readdir(root, { withFileTypes: true })) { const path = join(root, entry.name); const info = await lstat(path); if (info.isSymbolicLink()) throw new Error(`No se publican enlaces simbólicos: ${path}`); if (info.isDirectory()) result.push(...await listFiles(path)); else if (info.isFile()) result.push(path); } return result; }
