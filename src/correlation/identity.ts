...
export interface IdentityResolution { nodes: GraphNode[]; aliases: Map<string, string>; conflicts: Array<{ alias: string; candidates: string[] }>; }

export function resolveIdentities(facts: readonly Fact[], scenario: Scenario): IdentityResolution {
  const nodes = new Map<string, GraphNode>();
  const aliases = new Map<string, string>();
  const conflicts: Array<{ alias: string; candidates: string[] }> = [];
  ...
  for (const [alias, configuredTarget] of Object.entries(scenario.aliases)) {
    ...
    if (candidates.length === 1) aliases.set(normalizeAlias(alias), candidates[0]!);
    else conflicts.push({ alias, candidates });
  }
  for (const fact of facts) {
    if (!["message_resource", "data_resource", "package_dependency"].includes(fact.kind)) continue;
    ...
    const type: GraphNode["type"] = fact.kind === "message_resource" ? "message_resource" : fact.kind === "data_resource" ? "data_resource" : "package";
    const id = stableId("node", type, environment, label);
    const existing = nodes.get(id);
    if (existing) existing.fact_ids.push(fact.id); else nodes.set(id, { id, type, label, environment, fact_ids: [fact.id] });
  }
  return { nodes: [...nodes.values()], aliases, conflicts };
}

export function normalizeAlias(value: string): string {
  try { const url = new URL(value); return `${url.protocol}//${url.host}`.toLocaleLowerCase("en-US"); } catch { return value.trim().toLocaleLowerCase("en-US").replace(/\/$/u, ""); }
}
