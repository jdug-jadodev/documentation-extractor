...
export interface SemanticFlow {
  ...
  calls: SemanticCall[];
  data: Array<{ kind: string; entity: string; operation: string; source_path: string; evidence_ids: string[] }>;
  integrations: Array<{ method: string; target: string; source_path: string; evidence_ids: string[] }>;
  limitations: string[];
}
...
export function traceEndpointFlow(endpoint: ResolvedEndpoint, facts: readonly Fact[], maxDepth = 12, maxSymbols = 80, maxCalls = 160): SemanticFlow {
  ...
  const outgoing = new Map<string, SemanticCall[]>();
  for (const call of calls) outgoing.set(call.caller_symbol_id, [...(outgoing.get(call.caller_symbol_id) ?? []), call]);
  const start = resolveHandlerSymbol(endpoint, facts, symbols);
  const reached: SemanticSymbol[] = [], reachedCalls: SemanticCall[] = [], seen = new Set<string>();
  const queue: Array<{ id: string; depth: number }> = start === null ? [] : [{ id: start.id, depth: 0 }];
  while (queue.length > 0 && reached.length < maxSymbols) {
    ...
    for (const call of outgoing.get(current.id) ?? []) {
      if (reachedCalls.length < maxCalls) reachedCalls.push(call);
      if (call.target_symbol_id !== null && !seen.has(call.target_symbol_id)) queue.push({ id: call.target_symbol_id, depth: current.depth + 1 });
    }
  }
  ...
  if (start === null) limitations.push(`No se pudo enlazar el handler «${endpoint.handler}» con un símbolo AST.`);
  if (unresolved > 0) limitations.push(`${unresolved} llamada(s) del flujo conservan resolución candidata o no resuelta.`);
  if (reached.length >= maxSymbols) limitations.push(`El flujo se limitó a ${maxSymbols} símbolos para mantener una respuesta compacta.`);
  if (reachedCalls.length >= maxCalls) limitations.push(`La respuesta se limitó a ${maxCalls} llamadas para mantener el contexto compacto.`);
...
export function explainServiceFromFacts(runId: string, component: string, facts: readonly Fact[], graph: KnowledgeGraph, symbolLimit = 80) {
  ...
  const technologies = selected.filter((fact) => fact.kind === "technology" || fact.kind === "package_dependency").map((fact) => {
    const value = asRecord(fact.value); return String(value.package ?? value.technology ?? value.name ?? value.value ?? "");
  }).filter(Boolean);
  ...
  const allClasses = [...new Set(symbols.map((symbol) => symbol.class_name ?? (symbol.symbol_type === "class" ? symbol.name : null)).filter((value): value is string => value !== null))];
  let remaining = symbolLimit;
  const classes: Array<{ name: string; methods: Array<{ name: string; signature: string; description: string; source_path: string; start_line: number | null }> }> = [];
  for (const name of allClasses) { if (remaining <= 0) break; const methods = symbols.filter((symbol) => symbol.class_name === name).slice(0, remaining).map((symbol) => ({ name: symbol.name, signature: symbol.signature, description: symbol.description, source_path: symbol.source_path, start_line: symbol.start_line })); classes.push({ name, methods }); remaining -= Math.max(1, methods.length); }
...
function resolveHandlerSymbol(endpoint: ResolvedEndpoint, facts: readonly Fact[], symbols: readonly SemanticSymbol[]): SemanticSymbol | null {
...
function symbolFromFact(fact: Fact): SemanticSymbol | null {
...
function callFromFact(fact: Fact): SemanticCall | null {
...
function compactSymbol(symbol: SemanticSymbol) { return { id: symbol.id, class_name: symbol.class_name, name: symbol.name, type: symbol.symbol_type, signature: symbol.signature, description: symbol.description, source_path: symbol.source_path, start_line: symbol.start_line }; }
function operationFor(kind: string): string { return kind === "data_read" ? "read" : kind === "data_write" ? "write" : "reference"; }
function normalizePath(value: string): string { const normalized = value.trim().replace(/\/+$/u, ""); return normalized === "" ? "/" : normalized; }
