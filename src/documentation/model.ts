...
function factualSection(id: string, facts: Map<string, Fact[]>, graph: KnowledgeGraph, snapshots: Snapshot[]): Pick<DocumentSection, "paragraphs" | "claims" | "tables" | "limitations"> {
  ...
  if (id === "arquitectura" || id === "flujos") base.tables.push({ headers: ["Origen", "Relación", "Destino", "Estado", "Evidencia"], rows: graph.edges.map((edge) => [nodeLabel(graph, edge.from), edge.type, nodeLabel(graph, edge.to), edge.status, edge.evidence_ids.join(", ")]) });
  ...
  if (id === "evidencias") base.tables.push({ headers: ["Hecho", "Componente", "Tipo", "Valor observado", "Regla", "Evidencias"], rows: [...facts.values()].flat().map((fact) => [fact.id, fact.component_id, fact.kind, stringify(fact.value), fact.rule_id, fact.evidence_ids.join(", ")]) });
