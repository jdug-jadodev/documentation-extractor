...
export function renderRelationshipMermaid(model: DocumentModel): string {
  ...
  for (const row of rows) { const from = row[0] ?? "?", relation = row[1] ?? "relación", to = row[2] ?? "?", status = row[3] ?? "unknown", key = `${from}\u0000${relation}\u0000${to}\u0000${status}`; if (seen.has(key)) continue; seen.add(key); const arrow = status === "supported" ? "-->" : "-.->"; lines.push(`  ${id(from)}["${escapeMermaid(from)}"] ${arrow}|"${escapeMermaid(`${relation} · ${status}`)}"| ${id(to)}["${escapeMermaid(to)}"]`); }
  return lines.join("\n");
}
