...
import type { Snapshot } from "./contracts/types.js";
...
export interface RefreshKnowledgeResult {
  ...
  repositories: Array<RepositorySyncResult & { changed_paths: string[]; update_mode: "full" | "incremental" | "reused"; reprocessed_paths: string[]; reused_files: number }>;
  relations: number;
  ai_invocations: 0;
  published: boolean;
  publication?: unknown;
...
export async function refreshKnowledge(input: {
  ...
  if (repositoryIds.length === 0) throw new Error("No hay repositorios habilitados para actualizar.");
  const report = await preflight(config, repositoryIds);
  if (report.status !== "ready" || report.git_executable === null) throw new Error(report.issues.map((item) => item.message).join("; "));
  const refs = Object.fromEntries(repositoryIds.map((id) => {
    const repository = config.repositories.find((item) => item.id === id);
    if (repository === undefined) throw new Error(`Repositorio no autorizado: ${id}`);
    const ref = input.refs?.[id] ?? repository.default_branch;
    if (ref === null || ref.trim() === "") throw new Error(`La rama de ${id} no está configurada.`);
    return [id, ref];
  }));
  const baseline = await findCompatibleBaseline(config, repositoryIds, refs);
  const synchronized: RepositorySyncResult[] = [];
  for (const repositoryId of repositoryIds) {
    const ready = report.repositories.find((item) => item.id === repositoryId);
    if (ready === undefined) throw new Error(`Repositorio no disponible: ${repositoryId}`);
  ...
  if (baseline !== null && synchronized.every((item) => baseline.snapshots.some((snapshot) => snapshot.repository_id === item.repository_id && snapshot.commit_oid === item.commit_after))) {
    ...
    if (shouldPublish || baseline.published) await atomicWrite(join(config.state_root, "current-run.json"), `${JSON.stringify({ schema_version: 3, run_id: baseline.run_id, repositories: repositoryIds, refs, published: shouldPublish || baseline.published, updated_at: new Date().toISOString() }, null, 2)}\n`);
    return {
      schema_version: 3,
      status: "unchanged",
...
async function synchronizeBranch(input: { git: string; root: string; repositoryId: string; branch: string; syncRemote: boolean; signal?: AbortSignal }): Promise<RepositorySyncResult> {
  const valid = await git(input, ["check-ref-format", "--branch", input.branch], 4096);
  if (valid.exit_code !== 0) throw new Error(`Rama inválida para ${input.repositoryId}: ${input.branch}`);
  const before = await commit(input, input.branch);
  if (!input.syncRemote) return { repository_id: input.repositoryId, branch: input.branch, remote: null, commit_before: before, commit_after: before, fetched: false, pulled: false };
  const branch = await requiredGit(input, ["branch", "--show-current"], 4096, "No se pudo determinar la rama activa");
  if (branch.trim() !== input.branch) throw new Error(`${input.repositoryId}: la rama activa es ${branch.trim() || "HEAD separado"}; cambie a ${input.branch} antes de sincronizar. El motor no cambia ramas automáticamente.`);
  ...
  if (status.trim() !== "") throw new Error(`${input.repositoryId}: hay cambios locales sin commit. No se ejecutará pull ni se sobrescribirá el trabajo del desarrollador.`);
  ...
  if (remoteCheck.exit_code !== 0) throw new Error(`${input.repositoryId}: no existe el remoto ${remote} para sincronizar ${input.branch}.`);
  ...
  if (remoteCommit !== before) {
    ...
    if (ancestor.exit_code !== 0) throw new Error(`${input.repositoryId}: la rama local no puede avanzar por fast-forward hasta ${upstream}. Revise commits locales o una divergencia; el motor no hará reset ni merge forzado.`);
...
async function diffPaths(gitExecutable: string, root: string, base: string, target: string, signal?: AbortSignal): Promise<string[]> {
  if (!/^[0-9a-f]{40,64}$/u.test(base) || !/^[0-9a-f]{40,64}$/u.test(target)) throw new Error("OID inválido al calcular cambios incrementales.");
  ...
  if (result.exit_code !== 0) throw new Error(`No se pudo calcular el diff incremental: ${clean(result.stderr)}`);
  const fields = result.stdout.split("\0").filter(Boolean);
  const paths = new Set<string>();
  for (let index = 0; index < fields.length;) {
    const status = fields[index++]!;
    const first = fields[index++];
    if (first === undefined) throw new Error("Salida incompleta de git diff.");
    paths.add(first);
    if (/^[RC]/u.test(status)) { const second = fields[index++]; if (second === undefined) throw new Error("Renombre incompleto en git diff."); paths.add(second); }
  }
  return [...paths].sort();
}

async function commit(input: { git: string; root: string; repositoryId: string; branch: string; signal?: AbortSignal }, ref: string): Promise<string> {
  const value = (await requiredGit(input, ["rev-parse", "--verify", `${ref}^{commit}`], 4096, `No se pudo resolver ${ref}`)).trim();
  if (!/^[0-9a-f]{40,64}$/u.test(value)) throw new Error(`${input.repositoryId}: Git devolvió un commit inválido.`);
  return value;
}

async function git(input: { git: string; root: string; signal?: AbortSignal }, args: string[], maxStdout: number, timeout = 30_000) {
...
async function requiredGit(input: { git: string; root: string; signal?: AbortSignal }, args: string[], maxStdout: number, label: string, timeout?: number): Promise<string> {
  const result = await git(input, args, maxStdout, timeout);
  if (result.exit_code !== 0) throw new Error(`${label}: ${clean(result.stderr)}`);
  return result.stdout;
}

interface BaselineRun { run_id: string; snapshots: Snapshot[]; relations: number; published: boolean; }

async function findCompatibleBaseline(config: EffectiveConfiguration, repositories: readonly string[], refs: Readonly<Record<string, string>>): Promise<BaselineRun | null> {
  const preferred = await readJson<{ run_id?: string; published?: boolean }>(join(config.state_root, "current-run.json")).catch(() => null);
  const currentEdition = await readJson<{ run_id?: string }>(join(config.vault_root, "Actual", "edicion.json")).catch(() => null);
  const candidates = [...new Set([preferred?.run_id, currentEdition?.run_id, ...(await readdir(join(config.state_root, "runs"), { withFileTypes: true }).catch(() => [])).filter((item) => item.isDirectory()).map((item) => item.name).sort().reverse()].filter((item): item is string => typeof item === "string"))];
  for (const runId of candidates) {
    if (!/^run-[A-Za-z0-9._-]+$/u.test(runId)) continue;
    try {
      const run = await readJson<{ snapshots?: Snapshot[] }>(join(config.state_root, "runs", runId, "run.json"));
      const snapshots = run.snapshots ?? [];
      if (snapshots.length !== repositories.length || !repositories.every((id) => snapshots.some((snapshot) => snapshot.repository_id === id && snapshot.requested_ref === refs[id]))) continue;
      const graph = await readJson<{ edges?: unknown[] }>(join(config.state_root, "runs", runId, "graph.json"));
...
function nullDevice(): string { return process.platform === "win32" ? "NUL" : "/dev/null"; }
