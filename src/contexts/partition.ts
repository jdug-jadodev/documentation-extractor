...
export interface ContextPart { packet: TaskPacket; byte_length: number; estimated_tokens: number; omitted_fact_ids: string[]; }

export function partitionContext(packet: TaskPacket, maxBytes = 49_152): ContextPart[] {
  const base = { ...packet, facts: [], evidence: [] };
  const baseBytes = Buffer.byteLength(JSON.stringify(base), "utf8");
  if (baseBytes > maxBytes) throw new Error("La solicitud, perfiles o plantilla exceden el límite incluso sin hechos.");
  const evidenceById = new Map(packet.evidence.map((item) => [item.id, item]));
  const parts: ContextPart[] = [];
  let facts = [] as TaskPacket["facts"], evidence = [] as TaskPacket["evidence"];
  const flush = () => {
    if (facts.length === 0 && parts.length > 0) return;
  ...
  for (const fact of packet.facts) {
    ...
    if (Buffer.byteLength(JSON.stringify({ ...packet, facts: candidateFacts, evidence: candidateEvidence }), "utf8") > maxBytes) { if (facts.length === 0) throw new Error(`Un hecho individual excede el límite: ${fact.id}`); flush(); }
    facts.push(fact);
